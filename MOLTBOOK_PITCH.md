...
## HashKey Chain Horizon Hackathon — Project Submission

---

## Project Name
Guardian Protocol — Fail-closed security middleware for autonomous agents executing swaps on HashKey Chain.
...
1. **Token Risk Analyzer** — calls GoPlus Security API token scan on both input/output tokens; flags honeypots, predatory taxes, blacklist functions, and unverified contracts.
2. **TX Simulation Analyzer** — simulates via eth_call on HashKey Chain RPC, cross-validates against dual-RPC cross-validation, then runs 8-variant invariant fuzzing to detect state-dependent revert traps.
...
- **GoPlus Security API (Token Scan)** — Primary oracle for honeypot/tax/blacklist detection
- **GoPlus Security API (TX Pre-execution)** — Independent cross-validation of eth_call + fuzzing simulation
- **DEX API (Quote/Routing)** — Optimized routing for approved trades
- **Free-tier API auth** — Standard GoPlus API v5 authentication
- **HashKey Chain RPC (native viem client)** — 3-endpoint round-robin with 1500ms failover
...
- GitHub repo: https://github.com/bitnormie01/guardian-protocol (branch: main)
- Verified on-chain contract (GuardianProofLogger): `0x93A3DB5645Cb21DBDfEAB3047Fe01D1A65a8F52F`
- Explorer verification: https://www.oklink.com/hashkey-chain/address/0x93a3db5645cb21dbdfeab3047fe01d1a65a8f52f

## Why It Matters
Autonomous agents are executing real trades with real money on HashKey Chain right now. They have no eyes, no instincts, and no way to detect a honeypot from a legitimate token. Guardian gives every agent a security oracle it can call before every single trade — deterministic, machine-readable, fail-closed. It is the missing safety layer for the agentic DeFi stack. Any agent that integrates `@guardian-protocol/skill` gets institutional-grade trade security with a single import.
...
### 📝 Tagline
*Mainnet-hardened, fail-closed security middleware for autonomous agents on HashKey Chain — because the safest trade is the one you don't make, unless Guardian says it's clear.*

---

## 🎯 What It Does

Guardian Protocol is a deterministic security middleware built *for* autonomous AI agents (or human traders) to consume before executing swaps on HashKey Chain. Guardian:

1. **Scans both tokens** for honeypots, predatory taxes, blacklist functions, and unverified contracts using the **GoPlus Security API**.
2. **Simulates the transaction** with `eth_call` on HashKey Chain, **cross-validates** the result against **dual-RPC cross-validation**, and then runs **8-variant invariant fuzzing** to detect hidden revert traps and state-dependent manipulation.
...
- **Cross-validates** its own simulation against dual-RPC cross-validation. If they disagree, the trade is blocked.
...
### 2. Ecosystem Fit (HashKey Chain Integration) 🔗

Guardian is deeply integrated with HashKey Chain infrastructure:

| GoPlus Feature | How Guardian Uses It |
|---|---|
| **GoPlus Security API** (Token Scan) | Primary oracle for honeypot/tax/blacklist detection |
| **GoPlus Security API** (TX Pre-execution) | Independent cross-validation of our eth_call + fuzzing simulation |
| **DEX API** (Quote/Routing) | Optimized routing for approved trades |
| **Free-tier API auth** | Standard GoPlus API v5 authentication implemented |
| **HashKey Chain RPC** | Native viem-based client with 3-endpoint round-robin redundancy |
...
Guardian is built **for** the HashKey Chain ecosystem. It makes GoPlus security APIs accessible to autonomous agents who can't use a UI.
...
| Live Fire Demo | ✅ HashKey Chain Testnet (Chain ID 133) |
...
| **viem 2.21** | HashKey Chain RPC client, ABI encoding, contract reads |
...
| **GoPlus Security API** | Token risk + TX simulation oracle |
| **HashKey Chain** | Target blockchain (EVM-compatible L2) |
...
- **Dual-RPC results diverge?** → Penalty. Likely blocked.
...
# Run live fire against HashKey Chain Testnet
npm run live-fire

# Use the CLI
npx tsx src/cli.ts evaluate 0xTokenIn 0xTokenOut 1000000 --chain 133
```
...
Target Chain:         HashKey Chain Testnet (133)
```

The live fire test demonstrates:
1. **Full pipeline evaluation** — WHSK → USDC swap assessed and scored
...
**Guardian Protocol** — HashKey Chain Horizon Hackathon 2026 — AI Track
...
> HashKey Chain Mainnet (Chain ID 177) — [View on HashKey Explorer](https://www.oklink.com/hashkey-chain/address/0x6e9fb08755b837388a36ced22f26ed64240fb29c)
...
> HashKey Chain Mainnet (Chain ID 177) — [View on HashKey Explorer](https://www.oklink.com/hashkey-chain/address/0x93a3db5645cb21dbdfeab3047fe01d1a65a8f52f)
