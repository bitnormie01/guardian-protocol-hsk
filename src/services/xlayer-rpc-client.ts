// ==========================================================================
// Guardian Protocol — X Layer JSON-RPC Client (Phase 2: RPC Redundancy)
// ==========================================================================
//
// This module provides a typed client for interacting with X Layer's
// EVM-compatible JSON-RPC interface. It's the backbone of our
// transaction simulation engine.
//
// X Layer (chain ID 196) is an OKX-operated zkEVM L2. It supports
// standard Ethereum JSON-RPC methods including the critical ones we
// need for simulation:
//
...
//   (up from 500ms — X Layer RPC latency can exceed 500ms under load).
...
//   us a deterministic snapshot of chain state. The OKX onchain
//   gateway can also provide trace-level simulation via their
//   transaction-scan endpoint, which we integrate as a fallback.
// ==========================================================================
...
/**
 * X Layer Mainnet chain definition for viem.
 * Chain ID 196, OKB as native gas token.
 */
...
/**
 * X Layer Testnet chain definition for viem.
 */
 ...
 * Ordered by expected reliability (OKX official first).
 * Users can extend/override via XLAYER_RPC_URL, XLAYER_RPC_URL_2, XLAYER_RPC_URL_3 env vars.
 */
const DEFAULT_MAINNET_ENDPOINTS: string[] = [
  "https://rpc.xlayer.tech",
  "https://xlayerrpc.okx.com",
  "https://rpc.xlayer.tech", // Duplicate as final fallback (retried with fresh connection)
];
 ...
 * X Layer RPC endpoints. It wraps viem PublicClient instances and
...
export class RoundRobinRPCManager {
  ...
  async execute<T>(
    ...
    throw new GuardianError(
      ...
      `All ${orderedEndpoints.length} X Layer RPC endpoints failed for ` +
...
export class XLayerRPCClient {
  ...
  constructor(
    ...
    const chain = chainId === 196 ? xLayerMainnet : xLayerTestnet;
    const defaultEndpoints =
      chainId === 196 ? DEFAULT_MAINNET_ENDPOINTS : DEFAULT_TESTNET_ENDPOINTS;
    ...
    const envUrl1 = process.env["XLAYER_RPC_URL"];
    const envUrl2 = process.env["XLAYER_RPC_URL_2"];
    const envUrl3 = process.env["XLAYER_RPC_URL_3"];
    ...
    logger.debug("XLayerRPCClient initialized with round-robin RPC", {
      chainId,
   ...
   *   comes from decoding the swap's return data or from the OKX
  ...
  /**
   * Gets the native OKB balance of an address.
   */
  ...
  /**
   * Gets the current gas price on X Layer.
